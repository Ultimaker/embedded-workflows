...
#
# Centralized build script for embedded firmware projects
# This script orchestrates Docker-based builds with proper authentication
...
SRC_DIR="$(pwd)"
RELEASE_VERSION="${RELEASE_VERSION:-0.0.1-alpha.0}"
DOCKER_WORK_DIR="/build"
BUILD_DIR_TEMPLATE="_build_${ARCH}"
BUILD_DIR="${BUILD_DIR:-${SRC_DIR}/${BUILD_DIR_TEMPLATE}}"

# Docker configuration
DOCKER_IMAGE_NAME="${DOCKER_IMAGE_NAME:-opinicus}"
DOCKER_REGISTRY="${DOCKER_REGISTRY:-europe-west1-docker.pkg.dev}"
...
run_unittest()
{
    ...
    run_in_docker "./ci/local/run_unit_test.sh"
}

usage()
{
    echo "Usage: ${0} [OPTIONS]"
    echo "  -a ACTION  Specify action (build|build_docker|build_docker_cache|shellcheck|unittest)"
...
case "${action}" in
    ...
    build)
        ...
        ;;
    build_docker)
        # Build Docker image without loading it locally
        # This is typically used in CI/CD pipelines
        source ./docker_env/make_docker.sh ""
        exit 0
        ;;
    build_docker_cache)
        export DOCKER_BUILD_ONLY_CACHE="yes"
        source ./docker_env/make_docker.sh ""
        exit 0
        ;;
    unittest)
